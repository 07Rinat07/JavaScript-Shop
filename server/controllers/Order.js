    ...
    async create(req, res, next, type) {
        try {
            ...
            if (type === 'admin') {
                ...
                const basket = await BasketModel.getOne(parseInt(req.signedCookies.basketId, 10))
            ...
            // корзину очищаем только для user/guest сценариев
            if (type !== 'admin' && req.signedCookies.basketId) {
                await BasketModel.clear(parseInt(req.signedCookies.basketId, 10))
            }
            res.json(order)
        } catch(e) {
