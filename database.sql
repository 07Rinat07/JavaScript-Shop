...
2	Сергей Петров	user@mail.ru	+7 (701) 456-78-90	г Уральск, ул Строителей, дом 123, кв. 456	59732	0	\N	2022-03-16 14:58:07.621+03	2022-03-16 14:58:07.621+03	3
3	Сергей Петров	user@mail.ru	+7 (701) 456-78-90	г Уральск, ул Строителей, дом 123, кв. 456	128789	0	Позвонить перед доставкой	2022-03-16 14:59:59.507+03	2022-03-16 14:59:59.507+03	3
4	Николай Семенов	admin@mail.ru	+7 (707) 654-32-10	г Уральск, ул Космонавтов, дом 456, кв. 789	658377	0	\N	2022-03-16 16:22:44.765+03	2022-03-16 16:22:44.765+03	4
\.
...
--
-- Custom extension: content blocks for editable static pages
--
...
--
-- Custom extension: feedback forms
--
...
CREATE TABLE IF NOT EXISTS public.feedbacks (
    ...
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    updated_at timestamp with time zone NOT NULL DEFAULT now(),
    CONSTRAINT feedbacks_status_check CHECK (status IN ('new', 'read', 'spam')),
    CONSTRAINT feedbacks_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS public.feedback_blocks (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    email character varying(120) UNIQUE,
    ip character varying(80) UNIQUE,
    reason character varying(120) NOT NULL DEFAULT 'spam',
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    updated_at timestamp with time zone NOT NULL DEFAULT now(),
    CONSTRAINT feedback_blocks_contact_check CHECK (email IS NOT NULL OR ip IS NOT NULL)
);
